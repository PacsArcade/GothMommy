...
RegisterNetEvent('fx-idcard:server:GetData', function()
    ...
    exports.oxmysql:execute("SELECT * FROM fx_idcard WHERE charid = ?", {charid},function(result)
        if result[1] then
            ...
            -- Attach prev_license from history if present
            exports.oxmysql:execute("SELECT prev_license FROM fx_idcard_history WHERE charid = ?", {charid}, function(history)
                if history and history[1] and history[1].prev_license then
                    cardData.prev_license = history[1].prev_license
                end
